from server.server import list_tools, character_count, word_count
...
def test_word_count():
    ...
    )  # Default split handles extra whitespace
    assert word_count("oneword") == 1
    assert word_count("") == 0  # Splitting an empty string results in an empty list
    assert (
        word_count("   ") == 0
    )  # Splitting a string with only spaces results in an empty list
